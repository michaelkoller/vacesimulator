...
public class Cutter : MonoBehaviour
{
    ...
    public static GameObject Cut(GameObject _originalGameObject, Vector3 _contactPoint, Vector3 _direction, Material _cutMaterial = null, bool fill = true, bool _addRigidbody = false)
    {
        ...
        //CHANGED Michael Koller
        MeshCollider origMeshCollider = _originalGameObject.GetComponent<MeshCollider>();
        if (origMeshCollider == null)
        {
            Destroy(_originalGameObject.GetComponent<Collider>());
            origMeshCollider = _originalGameObject.AddComponent<MeshCollider>();
        }
        _originalGameObject.GetComponent<MeshFilter>().mesh = finishedLeftMesh;
        origMeshCollider.sharedMesh = finishedLeftMesh;
        origMeshCollider.convex = true;
        //END CHANGED Michael Koller
        ...
        rightGO.transform.position = _originalGameObject.transform.position; //+ (Vector3.up * .05f); //TODO set new position to center of new mesh --> add all verts
        ...
        origObjectId.ghosts[0].transform.GetComponent<MeshCollider>().sharedMesh = _originalGameObject.GetComponent<MeshCollider>().sharedMesh;
